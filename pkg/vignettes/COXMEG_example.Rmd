...
We fit a Cox mixed-effects model using the ```coxmeg``` function. We set ```type='bd'``` to indicate that the relatedness matrix is a block-diagonal matrix. However, the function will automatically treat it as dense if there are more than 50% non-zero elements in the matrix. Note that ```type='bd'``` should be used only for a block-diagonal matrix or a sparse matrix of which its inverse matrix is also sparse. For a general sparse relatedness matrix of which its inverse is not sparse, ```type='sparse'``` should be used. When ```type='sparse'``` is specified, the relatedness matrix will not be inverted during the estimation procedure. 
